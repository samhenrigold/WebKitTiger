...
`OptionsTigerProcesses.cmake` already sets **every feature flag identically for
all four configurations**, because any disagreement diverges the wire. The
per-process sections set only JIT switches and target selection.

**The authoritative list is `logs/tiger-ipc-shared-flags.txt`** — 221 lines,
every `ENABLE_`/`USE_` name appearing in a serialization or message condition
with its final value, generated by the configure from one grep so it cannot
drift. Each build directory carries its own copy as `tiger-ipc-features.txt`;
`tiger-check-ipc` compares two trees and fails by name. **Check any flag against
that file before proposing it move out of the shared set.**

Three consequences that constrain several briefs below:

- **Video cannot be a per-process choice, and is now already on.**
  `ENABLE_VIDEO` and `ENABLE_MEDIA_SOURCE` read `ON` in every configuration as of
  wkcmake's latest pass, because the ffmpeg backend is a whole-port decision.
  `ENABLE_GPU_PROCESS` is the most-used condition of all and reads `OFF` today;
  it flips for all four at once when the GPU process lands at N2.
- **55 of the names are compile-time macros in `PlatformEnable*.h`**, not CMake
  options, so making them agree is the port header's job. That is brief
  **N1-B**. They include `ENABLE_ROUTING_ARBITRATION`,
  `ENABLE_DOM_AUDIO_SESSION`, `ENABLE_UI_SIDE_COMPOSITING`,
  `ENABLE_TILED_CA_DRAWING_AREA`, and the PDF, accessibility, touch and
  notification ones.
- **`USE_CG` and `USE_CORE_TEXT` are among the 55**, and read `<unset>` in the
  record, confirming they are not CMake variables under the Cocoa port. The
  divergence we most want therefore cannot be expressed in the build system, so
  the answer is not matching flags but forcing the port-neutral encoding for the
  three types that change with them. **wkcmake has that switch on by default**,
  which narrows brief N1-B considerably.

### 0.3 Answered: four processes, and the four configurations were right

I raised this as an open question — `OptionsTigerProcesses.cmake` had four
`TIGER_PROCESS` values while the plan described three processes. **The four
configurations were correct and the plan was wrong**, now fixed
(`logs/split-process-plan.md` §2.0.2).

Replay runs in a **separate i386 GPU process** painting into shareable bitmaps;
the UI process keeps the compositor and uploads the tiles, which it must do in
either design. My objection to this — that a separate process would copy every
frame back — confused the compositor with the replayer. `spike/ipc32x64` measured
the UI-side GL upload **directly from the mapping at ~8 ms with no copy**, so the
split costs nothing there.

The merge would have cost ~6,000 relocated lines and, worse, **CoreGraphics
replay on a non-main thread beside AppKit, against a 2005 CoreGraphics whose font
and colour-space caches were never audited for that** — the likely source of
intermittent crashes, and a property of Tiger rather than of WebKit, so no
upstream structure would have saved us.
