...
rsync -t "$WKT/spike/fasttext/ctref32int" tiger-eth:/tmp/fasttext/
rsync -t "$WKT/spike/fasttext/sample.html" tiger-eth:/tmp/fasttext/www/
# One ssh session per step so a hang is visible and bounded. 10.4 has no timeout(1); perl's alarm does.
ssh tiger-eth 'cd /tmp/fasttext && ./ctref32int ref smooth 2>&1 | tail -1'
...
for v in before after; do
    ssh tiger-eth "cd /tmp/fasttext && TIGER_FONT_MANIFEST=/Users/shg/wk2/share/tiger-fonts.json perl -e 'alarm 28; exec @ARGV' ./after/pagedriver ./$v/TigerWebProcess ./after/TigerNetworkProcess http://127.0.0.1:8392/sample.html /tmp/fasttext/$v.png > /tmp/fasttext/$v.log 2>&1; grep -E 'wrote|FAILED|timed out|DidFinishLoad' /tmp/fasttext/$v.log | tail -3; killall TigerWebProcess TigerNetworkProcess pagedriver 2>/dev/null || true"
done
