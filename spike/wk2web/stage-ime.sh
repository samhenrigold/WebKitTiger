#!/bin/sh
# stage-app.sh for the text-input track: build/tiger-{ui,web,gpu}-ime, staged to /Users/shg/wk2ime.
...
APP_ENV="HOME=/Users/shg/wk2ime/home TIGER_FONT_MANIFEST=/Users/shg/wk2ime/share/tiger-fonts.json TIGER_CA_BUNDLE=/Users/shg/wk2ime/share/cacert.pem $APP_ENV"
...
for i in $(seq 1 60); do
    busy=$(ssh tiger-eth "ps -axo pid,command | grep -E '[T]iger(WK2App|Browser2|WebProcess|NetworkProcess|GPUProcess)|[p]agedriver|[/]Applications/TigerBrowser.app' | head -3")
    load=$(ssh tiger-eth "sysctl -n vm.loadavg" | tr -d '{}' | awk '{print int($1)}')
    [ -z "$busy" ] && [ "${load:-0}" -ge 4 ] && busy="load $load"
    [ -z "$busy" ] && break
...
ssh tiger-eth 'mkdir -p /Users/shg/wk2ime/bin /Users/shg/wk2ime/Frameworks /Users/shg/wk2ime/share /Users/shg/wk2ime/home'
rsync -t -z "$WKT/logs/tiger-fonts.json" "$WKT/deps/src/cacert.pem" "$WKT"/spike/wk2web/*.html tiger-eth:/Users/shg/wk2ime/share/ 2>/dev/null || echo "stage-app: share files not all copied (cacert.pem present?)"
...
UIDIR=${UIDIR:-build/tiger-ui-ime}; WEBDIR=${WEBDIR:-build/tiger-web-ime}; GPUDIR=${GPUDIR:-build/tiger-gpu-ime}
...
rsync -t -z --partial --inplace --bwlimit=20000 $FILES tiger-eth:/Users/shg/wk2ime/bin/
# Always sync the framework: the rebased QuartzCore changed twice today (rebase-dylib passes).
rsync -rtl -z --partial --inplace --bwlimit=20000 "$WKT/spike/CAHost/Frameworks/QuartzCore.framework" tiger-eth:/Users/shg/wk2ime/Frameworks/
...
ssh tiger-eth "cd /Users/shg/wk2ime/bin && (perl -e 'alarm $((SECS + 3)); exec @ARGV' -- env $APP_ENV ./$APP '$URL' $SECS -WebKitLogging Process,Loading > /Users/shg/wk2ime/app.log 2>&1 &) ; sleep 8; echo == procs at 8s; ps -axo pid,rss,%cpu,command | grep -E 'Tiger(WK2App|Browser2|WebProcess|NetworkProcess|GPUProcess)' | grep -v grep | cut -c1-90; netstat -m | grep 'clusters in use'; sleep $((SECS - 11)); screencapture -x /Users/shg/wk2ime/shot.png; sleep 5; ps -axo pid,command | awk '\$2 ~ /^\.\/Tiger/ || \$2 ~ /\/wk2ime\// {print \$1}' | xargs kill 2>/dev/null; sleep 1; echo == after; ps -axo pid,command | grep -E 'Tiger(WK2App|Browser2|WebProcess|NetworkProcess|GPUProcess)' | grep -v grep; netstat -m | grep -E 'clusters in use|denied'; true"
scp -qO tiger-eth:/Users/shg/wk2ime/shot.png "$SHOT"
if [ -n "$LOG" ]; then scp -qO tiger-eth:/Users/shg/wk2ime/app.log "$LOG"; fi
echo "== app.log"; ssh tiger-eth 'tail -40 /Users/shg/wk2ime/app.log'
