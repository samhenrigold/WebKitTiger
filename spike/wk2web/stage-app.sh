...
scp -qO "$WKT/build/tiger-ui-port/bin/TigerWK2App" tiger:/tmp/wk2/bin/
scp -qO "$WKT/build/tiger-web-port/bin/TigerWebProcess" "$WKT/build/tiger-web-port/bin/TigerNetworkProcess" tiger:/tmp/wk2/bin/
...
ssh tiger "cd /tmp/wk2/bin && (./TigerWK2App '$URL' $SECS > /tmp/wk2/app.log 2>&1 &) ; sleep $((SECS - 3)); screencapture -x /tmp/wk2/shot.png; sleep 4; ps -axo pid,rss,command | grep -E 'Tiger(WK2App|WebProcess|NetworkProcess)' | grep -v grep; true"
