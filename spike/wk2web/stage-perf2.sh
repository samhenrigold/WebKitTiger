...
GPUDIR=${GPUDIR:-build/tiger-gpu}   # GPUDIR=build/tiger-ui-perf stages a GPU process built next to the UI
TAG=${TAG:-perf2}
...
FILES=""
for f in "$WKT/build/tiger-ui-perf/bin/TigerWK2App" "$WKT/build/tiger-ui-perf/bin/TigerBrowser2" "$WKT/$WEBDIR/bin/TigerWebProcess" "$WKT/$WEBDIR/bin/TigerNetworkProcess" "$WKT/$GPUDIR/bin/TigerGPUProcess" "$WKT/build/tigeraudio32"; do
...
rsync -rtl -z --partial --inplace --bwlimit=20000 "$WKT/spike/CAHost/Frameworks/QuartzCore.framework" tiger-eth:/Users/shg/wk2perf/Frameworks/
