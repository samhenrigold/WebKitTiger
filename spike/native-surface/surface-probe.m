/* Derived from committed direct-present probe 7566cf6, kept independent of its
 * evolving performance experiments. Tiger-only experiment: render from a separate 32-bit process into an owned
...
@interface NativeBackingView : NSView
@end
@implementation NativeBackingView
...
- (void)drawRect:(NSRect)rect
{
    ...
    NSRectFill(NSMakeRect(0, 0, 1280, 36));
    [[NSColor colorWithCalibratedRed:1 green:0 blue:1 alpha:1] set];
    NSRectFill(NSMakeRect(430, 70, 180, 80));
}
@end

@interface NativeDocumentView : NSView
@end
@implementation NativeDocumentView
- (BOOL)isFlipped { return YES; }
- (BOOL)isOpaque { return NO; }
@end
...
static int renderChild(int fd, int useCA)
{
    ...
    GLint order = target.surfaceOrder, actualOrder = 0;
    requireOK("public-surface-order", CGLSetParameter(context, kCGLCPSurfaceOrder, &order));
    requireOK("read-surface-order", CGLGetParameter(context, kCGLCPSurfaceOrder, &actualOrder));
    fprintf(stderr, "NATIVE order requested=%d actual=%d\n", (int)order, (int)actualOrder);
    ...
    // This checks overlap, not throughput. Leave startup headroom and retain the
    // final complete picture across the runner's screenshot at ten seconds.
    enum { frameCount = 180 };
    ...
    sleep(6); // Keep the rendered surface alive for the later screenshot.
    if (texture)
...
int main(int argc, char** argv)
{
    ...
    int useCA = 0;
    int above = argc == 2 && !strcmp(argv[1], "--above");
    if (argc != 1 && !above)
        return 2;
#if !SURFACE_PROBE_CA
    if (useCA)
    ...
    requireOK("foreground-process", TransformProcessType(&psn, kProcessTransformToForegroundApplication));
    [NSApplication sharedApplication];
    [NSApp finishLaunching];
    NSWindow* window = [[NSWindow alloc] initWithContentRect:NSMakeRect(100, 100, 1280, 720)
        styleMask:NSTitledWindowMask backing:NSBackingStoreBuffered defer:NO];
    [window setTitle:@"Native Cocoa controls above a shared GPU drawable"];
    [window setOpaque:NO];
    [window setBackgroundColor:[NSColor clearColor]];
    NativeBackingView* backing = [[NativeBackingView alloc] initWithFrame:NSMakeRect(0, 0, 1280, 720)];
    [window setContentView:backing];
    [backing release];
    NSTextField* field = [[NSTextField alloc] initWithFrame:NSMakeRect(40, 72, 360, 28)];
    [field setStringValue:@"Before native editing"];
    [field setEditable:YES];
    [field setSelectable:YES];
    [backing addSubview:field];
    NSScroller* scroller = [[NSScroller alloc] initWithFrame:NSMakeRect(1240, 60, 16, 500)];
    [scroller setEnabled:YES];
    [scroller setFloatValue:0.35 knobProportion:0.28];
    [backing addSubview:scroller];
    NSClipView* clip = [[NSClipView alloc] initWithFrame:NSMakeRect(40, 150, 240, 72)];
    [clip setDrawsBackground:NO];
    NativeDocumentView* document = [[NativeDocumentView alloc] initWithFrame:NSMakeRect(0, 0, 420, 140)];
    NSTextField* clipped = [[NSTextField alloc] initWithFrame:NSMakeRect(160, 20, 240, 28)];
    [clipped setStringValue:@"CLIPPED native text beyond the viewport"];
    [document addSubview:clipped];
    [clip setDocumentView:document];
    [backing addSubview:clip];
    [clipped release]; [document release]; [clip release];
    [window makeKeyAndOrderFront:nil];
    [NSApp activateIgnoringOtherApps:YES];
    requireOK("front-process", SetFrontProcess(&psn));
    [window display];
    int connection = CGSMainConnectionID(), childConnection = 0;
    transfer(channel[0], &childConnection, sizeof(childConnection), 0);
    struct Surface target = { [window windowNumber], 0, 1280, 720, programStart, programStart + 8.5, above ? 1 : -1 };
    ...
    close(channel[0]);
    NSPoint origin = [window convertBaseToScreen:NSMakePoint(0, 0)];
    fprintf(stderr, "NATIVE geometry origin=%.0f,%.0f size=1280,720 field=40,72,360,28 clipped=200,170,80,28 scroller=1240,60,16,500\n",
        origin.x, NSMaxY([[NSScreen mainScreen] frame]) - origin.y - 720);
    ...
    while (CFAbsoluteTimeGetCurrent() < end) {
        ...
        if (!edited && CFAbsoluteTimeGetCurrent() - programStart > 1) {
            [window makeFirstResponder:field];
            [field selectText:nil];
            NSTextView* editor = (NSTextView*)[window firstResponder];
            if (![editor isKindOfClass:[NSTextView class]])
                return 4;
            [editor insertText:@"Native Cocoa text"];
            [editor setSelectedRange:NSMakeRange(0, 6)];
            NSRange selection = [editor selectedRange];
            BOOL matches = [[editor string] isEqualToString:@"Native Cocoa text"];
            fprintf(stderr, "NATIVE editing field-editor=%d text-match=%d selected=%lu,%lu key=%d clipped-viewport=240x72\n",
                [editor isFieldEditor], matches, (unsigned long)selection.location, (unsigned long)selection.length, [window isKeyWindow]);
            if (![editor isFieldEditor] || !matches || selection.location != 0 || selection.length != 6 || ![window isKeyWindow])
                return 4;
            edited = YES;
        }
        [NSApp updateWindows];
