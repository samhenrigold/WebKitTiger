...
SHOT_AT=${SHOT_AT:-12}
SCRIPT=${SCRIPT:-}   # TIGER_SCRIPT steps, e.g. SCRIPT='wait 25; click 500,357'
WEBBIN=${WEBBIN:-$WKT/build/tiger-web-media}
UIBIN=${UIBIN:-$WKT/build/tiger-ui-port}
OUT=${OUT:-}   # set to bring the whole timestamped app.log back here
...
[ -n "$OUT" ] && { mkdir -p "$(dirname "$OUT")"; scp -qO tiger-eth:/Users/shg/wk2media/app.log "$OUT"; echo "log: $OUT ($(grep -c TIGER-SAMPLE "$OUT") samples)"; }
