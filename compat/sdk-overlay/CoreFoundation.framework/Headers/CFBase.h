...
/* TIGER SDK OVERLAY: the CF_* annotation and enum macros, none of which the
   10.4 SDK has. CF_ENUM and CF_OPTIONS arrived in 10.8 and are the load-bearing
   pair -- without them "typedef CF_ENUM(CFIndex, Name) { ... }" parses as a
   function definition declared typedef, which is how PAL's SPI headers fail.
   The rest are compile-time annotations: bridging, ownership and nullability.

   Same shape as the NS_* macros in <TigerCompat/FoundationCompat.h>, and for
