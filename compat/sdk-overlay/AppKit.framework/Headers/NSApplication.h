/*	NSApplication.h -- TIGER SDK OVERLAY
	The 10.4u SDK's header with one instance variable annotated.

	"id *_hiddenList;" is "pointer to non-const type 'id' with no explicit
	ownership" under ARC, which is a hard error, and the Cocoa port compiles
	WebKit with ARC. __unsafe_unretained says what was always true of a 2005
	private ivar. Together with NSNetServices.h in this overlay, these are the
	only two "id *" instance variables in the 10.4 AppKit and Foundation
	headers. */
...
@interface NSApplication : NSResponder
{
    ...
    id                  _mainWindow;
    id                  _delegate;
    __unsafe_unretained id *_hiddenList;   /* TIGER: see header comment */
    int                 _hiddenCount;
    void		*_context;
