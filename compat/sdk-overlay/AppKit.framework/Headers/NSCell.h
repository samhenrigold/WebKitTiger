/*	NSCell.h -- TIGER SDK OVERLAY
	The 10.4u SDK's header plus the 10.10 NSControlSize renames.

	Same shape as the NSEvent and NSWindowStyleMask renames: a respelling with
	no value changes, so these are macros onto the existing enumerators and
	nothing is redeclared. 67 sites across the tree use the type. */
...
/* 10.16. No Tiger equivalent, and none of the three existing enumerators means
   "large", so it gets its own value rather than aliasing one of them. Tiger's
   AppKit never produces it, so a control asked for this size draws regular;
   aliasing regular would make the two indistinguishable in a comparison, which
   is worse. */
