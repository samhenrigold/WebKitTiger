/*	NSEvent.h -- TIGER SDK OVERLAY
	The 10.4u SDK's header, unchanged, followed by the 10.12 renames.

	In 10.12 Apple respelled every NSEventType, NSEventMask and modifier-flag
	constant without changing a single value. WebKit uses the new spelling at
	189 sites across about 20 files and the old spelling at none, so the
	mapping is total and mechanical. Macros rather than new enumerators, so
	both spellings name the same constant and nothing is redeclared.

	The 10.7+ NSEvent *methods* (-phase, -momentumPhase, -scrollingDeltaX/Y,
	-hasPreciseScrollingDeltas, -stage, -CGEvent) are not here: they need
	implementations, and live in <TigerCompat/AppKitCompat.h> and
	compat/nscompat-appkit.m. */
...
#define NSEventMaskAppKitDefined        NSAppKitDefinedMask
#define NSEventMaskSystemDefined        NSSystemDefinedMask
#define NSEventMaskApplicationDefined   NSApplicationDefinedMask
#define NSEventMaskPeriodic             NSPeriodicMask
#define NSEventMaskCursorUpdate         NSCursorUpdateMask
#define NSEventMaskScrollWheel          NSScrollWheelMask
#define NSEventMaskTabletPoint          NSTabletPointMask
#define NSEventMaskTabletProximity      NSTabletProximityMask
...
/* ---- Modifier flags, renamed in 10.12 -------------------------------- */
...
/* 10.6 named the flag word; Tiger passes it as a bare unsigned int. */
typedef unsigned int NSEventModifierFlags;

/* ---- NSEventPhase, 10.7 ----------------------------------------------
   The type only. Tiger's scroll events carry no phase information, so the
   accessors in <TigerCompat/AppKitCompat.h> answer NSEventPhaseNone and
   WebCore takes its non-precise scrolling path. */
enum {
    NSEventPhaseNone        = 0,
...
typedef unsigned int NSEventPhase;
