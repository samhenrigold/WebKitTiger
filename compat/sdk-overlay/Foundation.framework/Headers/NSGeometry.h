...
/* TIGER SDK OVERLAY: NSPoint, NSSize and NSRect are the CoreGraphics types.

   The 10.4 SDK declares them as separate structs, struct _NSPoint and friends.
   Apple unified them in 10.5 behind NS_BUILD_32_LIKE_64 and made it
   unconditional for 64-bit; WebCore assumes the unified world and does
   "typedef CGPoint NSPoint" of its own in FloatPoint.h, IntRect.h and four more
   headers, which against this SDK is "typedef redefinition with different types".

   This is safe here rather than merely convenient: on i386 struct _NSPoint is
   {float x; float y;} and struct CGPoint is {float x; float y;}, byte for byte,
   so every AppKit entry point taking an NSRect by value receives exactly the
   same four floats it did before. The only observable difference is the
   Objective-C type encoding, "{CGPoint=ff}" rather than "{_NSPoint=ff}", which
   matters only to code that compares encoding strings at runtime. */
