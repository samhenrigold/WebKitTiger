...
@private
    id _netService;
    id _delegate;
    /* TIGER SDK OVERLAY: an `id *` instance variable is "pointer to non-const
       type 'id' with no explicit ownership" under ARC, which is a hard error,
       and the Cocoa port compiles WebKit with ARC. This is a private reserved
       slot that nothing reads, so __unsafe_unretained is both correct and what
       Apple's own later headers say. */
    __unsafe_unretained id * _reserved;
}
