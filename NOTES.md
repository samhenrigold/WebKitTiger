...
## 2026-09-21 03:17 — three reports (coordinator)

- ctcompat 454d6d9/d38b183: all 7 textpixel cases pixel-identical (worst 0.0039 pt, asserted tol 0.02). Two rules now held by the harness: (1) AAT Apple-format kern → shape with -kern, apply pair value to the LEADING glyph; (2) shape HarfBuzz at 1/1024 pt, not the conventional 1/64 — 1/64 quantisation accumulates ~0.14 pt by glyph 18 on Helvetica 16 pt; CoreText computes in float. Rule 2 is platform-independent and belongs in the 64-bit FontCascade. Colour trap annotated at the three CT draw sites in compat/ctcompat.c (CTLineDraw ignores context fill on Tiger, reads kCTForegroundColorAttributeName). Proceeding to FontCache/FontCascade.
...
## 2026-09-21 — the i386 census ends: libWebKit.a and bin/TigerGPUProcess link

Coordinator took the census over from gpu32b after pass 77 (the agent had been
interrupted with a correct batch uncommitted; passes 70/73/74 had the identical
32-target failure set and 75/76 regressed on a broken PCH). Passes 78–87 in
`logs/gpu32-pass-*.log`. Method change: targeted ninja on the failed objects, full
passes only when the list was empty; and the library stopped compiling code the
i386 processes never link.

**What the i386 library is now.** UI + GPU processes only. `WebKit_SOURCES_EXCLUDE_REGEX`
(hook at the end of Source/WebKit/CMakeLists.txt, before the per-process subtargets)
drops `WebProcess/` and `NetworkProcess/` — unified bundles, Curl.cmake/WC.cmake
hand-listed halves, generated message receivers — and the GPU tree drops
`UIProcess/` too. `WebKit_SOURCES_EXCLUDE_KEEP` brings back the two WebProcess/ files
the GPU process shares (WCBackingStore, ImageBufferShareableBitmapBackend). The UI
census (pass 80/81 showed ~15 UIProcess unified sources failing behind the
`-ferror-limit`) is the UI tree's, TIGER_PROCESS=UI.

**Wire.** RetainPtr<CGColorSpaceRef> gets `Shared/tiger/ArgumentCodersTigerCG`,
which writes the one PlatformColorSpace::Name byte the no-CG peer writes. The Cocoa
CoreIPCCGColorSpace was tried and rejected: it is NSSecureCoding all the way down.
ImageBufferBackendHandle's alias copies in WebPage.h / WebPageProxy.h now match the
canonical header (`PLATFORM(COCOA) && HAVE(IOSURFACE)`; HAVE_IOSURFACE is #undef'd
for Tiger at PlatformHave.h:2068).

**Link.** 283 tombstones in `GPUProcess/tiger/Tombstones.c` + 5 empty ObjC classes,
generated by `tools/make-tombstones.py` from the ld report mapped through
`nm -u` + `c++filt` (destructors have two mangled variants that demangle alike; emit
every variant that no archive defines). Each is 256 × 0xCC so a call that reaches one
traps. Real definitions instead of tombstones: WTF unix fd helpers
(UniStdExtrasUnix.cpp), InitializeWebKit2 generic arm, WebCorePersistentCoders' CURL
arm for CertificateInfo. CFNetwork is a CoreServices subframework on 10.4: link
`-framework CoreServices`, headers via explicit `-F …/CoreServices.framework/Frameworks`
(toolchain INIT flags + add_compile_options for already-configured trees).
GPUProcess compiles `-fPIC` (not CMake's `-fPIE`) to reuse the library PCH; i386
clang rejects the PCH otherwise ("is pie differs"). bin/jsc and the x86_64
processes' `_CodeSign` ALL targets leave the i386 build.

**Box.** `/tmp/gpu/bin/TigerGPUProcess` with `spike/CAHost/Frameworks/QuartzCore.framework`
